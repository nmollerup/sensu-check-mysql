...
func TestExecuteCheckDSNGeneration(t *testing.T) {
	tests := []struct {
		...
		{
			name:             "TCP connection with lag thresholds",
			socket:           "",
			hostname:         "localhost",
			port:             3306,
			database:         "test",
		...
		{
			name:             "Unix socket connection with master connection name",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			plugin.MasterConnection = tt.masterConnection
